...
class GATModel(nn.Module):
    ...
    def forward(self, data):
        ...
        #print(x[root_index].reshape(-1,x.shape[-1]))
...
class TCCNN(torch.nn.Module):
    def __init__(
        ...
        self.mlp = MLP([1024+2*hidden_dim1, 512, 256, 2], dropout=dropout, norm=None)
    ...
    def forward(self, data):
        ...
        feature=torch.cat([point_feature,cnn_feature],dim=-1)
...
model_name="experiment/ourswouserencoder"
...
config.seed = 2121 #@param {type:"number"}
...
lstm_lr = config.learning_rate       # 设置gat模块的学习率
