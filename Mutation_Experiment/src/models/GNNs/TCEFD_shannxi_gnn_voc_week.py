...
from src.models.GNNs.Shannxi_Datasets import Graph_Dataset8
...
class GATModel(nn.Module):
    ...
    def get_true_index(self, edge_index):
        change_positions=[0]
        x_tensors=[]
        y_tensors=[]
        lengths=[]
    ...
    def forward(self, data):
        x, edge_index= data.x_temp.type(torch.float32), data.edge_index
        x1=x[:,:69]
        x2=x[:,96:]
        x = torch.cat((x1, x2), dim=1)  # 切片位置69-96
        #dataloader 返回的index是错误的，需要重新修改
        edge_index=self.get_true_index(edge_index)
...
class TCCNN(torch.nn.Module):
    def __init__(
        ...
        self.mlp = MLP([out_features*num_heads, 512, 256, 2], dropout=dropout, norm=None)

    def forward(self, data):

        gat_feature=self.gat(data)

        return self.mlp(gat_feature).log_softmax(dim=-1)
...
model_name="experiment/gat_voc_week"
