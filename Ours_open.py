...
class PointNetPlusPlus(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.sa1_module = SetAbstraction(set_abstraction_ratio_1, set_abstraction_radius_1, MLP([10+3, 64, 64, 128]))
...
class TCCNN(torch.nn.Module):
    def __init__(
        ...
        self.mlp = MLP([1024+out_features*num_heads, 512, 256, 2], dropout=dropout, norm=None)
        self.gat = MLP([num_features, hide_features*num_heads, out_features*num_heads], dropout=dropout, norm=None)
    ...
    def forward(self, data):
        ...
        x= data.x_temp.type(torch.float32)[:,:29]
        root_index= torch.nonzero(data.root).squeeze()
        gat_feature=self.gat(x[root_index].reshape(-1,x.shape[-1]))
        feature=torch.cat([point_feature,gat_feature],dim=-1)
...
def train_step(epoch):
    ...
    for batch_idx in progress_bar:
        ...
        if batch_idx!=0 and batch_idx%50==0:
            ...
            dic.update(test_step(epoch+batch_idx/num_train_examples))
            wandb.log(dic)
            model.train()
...
def save_checkpoint(epoch,args):
    ...
    checkpoint_artifact.add_file(args.mode+"checkpoint.pt")
    wandb.log_artifact(
...
# 添加命令行参数
parser.add_argument('--wandb_project', type=str, default="dianxin_open_ours")
...
parser.add_argument('--mode', type=str, default='06', help='比例选择')
...
train_dataset = Graph_Dataset6(
    root='Shannxi_Graph/',
    mode='train_name_'+args.mode,
    pre_filter=None,
    pre_transform=None
)
...
val_dataset = Graph_Dataset6(
    root='Shannxi_Graph/',
    mode='val_name_'+args.mode,
    pre_filter=None,
    pre_transform=None
)
...
test_dataset = Graph_Dataset6(
    root='Shannxi_Graph/',
    mode='test_name_'+args.mode,
    pre_filter=None,
    pre_transform=None
)
...
# Define Optimizer
# 定义不同模块的学习率
point_net_lr = config.learning_rate*5  # 设置point_net模块的学习率
gat_lr = config.learning_rate       # 设置gat模块的学习率
