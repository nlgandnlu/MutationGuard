...
class TCCNN(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.svm = SVM(input_size=64, num_classes=2)

    def forward(self, data):
        root_index= torch.nonzero(data.root).squeeze()
        svm_feature=self.svm(data.x_temp[root_index].reshape(-1,data.x_temp.shape[-1]).type(torch.float32))
        return svm_feature
...
# Define TCCNN model.
model = SVC(probability=True,kernel='linear', degree=3)  # 设置 probability=True 以获取预测概率 ,kernel='sigmoid' ,kernel='rbf' kernel='poly', degree=3 kernel='linear'
...
model = SVC(probability=True,kernel='poly', degree=3)  # 设置 probability=True 以获取预测概率 ,kernel='sigmoid' ,kernel='rbf' kernel='poly', degree=3 kernel='linear'
...
dic.update(change_step(epoch,X_change, y_change))
dic.update(fruad_step(epoch,X_fraud, y_fraud))
dic['model_name']='svm_poly'

save_csv(dic)
model = SVC(probability=True,kernel='sigmoid', degree=3)  # 设置 probability=True 以获取预测概率 ,kernel='sigmoid' ,kernel='rbf' kernel='poly', degree=3 kernel='linear'
...
dic.update(change_step(epoch,X_change, y_change))
dic.update(fruad_step(epoch,X_fraud, y_fraud))
dic['model_name']='svm_sigmoid'

save_csv(dic)
model = SVC(probability=True,kernel='rbf', degree=3)  # 设置 probability=True 以获取预测概率 ,kernel='sigmoid' ,kernel='rbf' kernel='poly', degree=3 kernel='linear'
