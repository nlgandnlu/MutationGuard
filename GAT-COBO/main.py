...
parser.add_argument('--dataset', type=str, default='Shaanxi', help='Sichuan,Shaanxi')
...
if args.dataset == 'Shaanxi':
    dataset, _ = load_graphs("./data/Shaanxi_tele.bin")
    n_classes = load_info("./data/Shaanxi_tele.pkl")['num_classes']
    graph = dataset[0]
    ...
    args.epochs = 2000
    args.lr = 0.01
    fraud_mask = g.ndata['fraud_mask'].to(device)
    mutation_mask = g.ndata['mutation_mask'].to(device)
...
sample_weights = torch.ones(g.adj().shape[0]).to(device)
...
if args.dataset == 'Shaanxi':
    fraud_h = torch.argmax(results[fraud_mask], dim=1)
    fraud_acc = torch.sum(fraud_h == labels[fraud_mask]) * 1.0 / len(labels[fraud_mask])
    mutation_h = torch.argmax(results[mutation_mask], dim=1)
    mutation_acc = torch.sum(mutation_h == labels[mutation_mask]) * 1.0 / len(labels[mutation_mask])
    print('*************fraud and mutation acc******************\n',fraud_acc, mutation_acc)
